"""深度学习从零训练: TextCNN + BiGRU-Attention
用法: python scripts/03_train_dl.py [--model textcnn|bigru_attn|all]
"""
...
def train_model(model_type: str, embedding_matrix, word2idx, class_weights):
    ...
    # 训练集用 WeightedRandomSampler 过采样少数类
    train_loader = create_weighted_train_loader(
        TRAIN_PATH, word2idx, BATCH_SIZE_DL, MAX_SEQ_LEN
    )
    val_loader = create_data_loader(
        VAL_PATH, word2idx, BATCH_SIZE_DL, MAX_SEQ_LEN, shuffle=False
    )
    test_loader = create_data_loader(
        TEST_PATH, word2idx, BATCH_SIZE_DL, MAX_SEQ_LEN, shuffle=False
    )
    ...
    plot_confusion_matrix(
        np.array(test_metrics["confusion_matrix"]), ["差评", "中评", "好评"],
        save_path=str(FIGURE_DIR / f"{model_type}_confusion_matrix.png"),
        title=f"{model_type} 混淆矩阵",
    )
    # 保存评估报告
